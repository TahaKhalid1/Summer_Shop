...
namespace Next_ae.User
{
    public partial class HomeMaster : System.Web.UI.MasterPage
    {
        ...
        protected void moblnkLogout_Click(object sender, EventArgs e)
        {
            // Your logout logic
            Session.Clear();
            Session.Abandon();
        ...
        public void mobUpdateLoginStatus()
        {
            if (Session["CustomerName"] != null)
            {
                mobpnlLoggedIn.Visible = true;
                mobpnlLoggedOut.Visible = false;
                moblblLoginStatus.Text = Session["CustomerName"].ToString();

                // Display full name but keep username in session
                if (Session["FirstName"] != null)
                {
                    moblblLoginStatus.Text = Session["FirstName"].ToString();
                }
            }
            else
            {
                mobpnlLoggedIn.Visible = false;
                mobpnlLoggedOut.Visible = true;
                moblblLoginStatus.Text = "Login";
            }
        }

        // Public method to refresh cart from content pages
